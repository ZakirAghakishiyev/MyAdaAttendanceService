...
{
    Task<SessionAttendance?> GetBySessionAndStudentAsync(int sessionId, Guid studentId);

    Task<List<SessionAttendance>> GetBySessionIdAsync(int sessionId);

    Task<List<SessionAttendance>> GetStudentAttendanceAsync(Guid studentId, int lessonId);
